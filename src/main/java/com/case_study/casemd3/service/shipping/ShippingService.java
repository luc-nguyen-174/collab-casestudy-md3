...
public class ShippingService implements IShipping {
    ...
    public static final String SELECT_ALL_FROM_SHIPPING_PARTNERS = "SELECT * FROM shipping_partners WHERE is_active = true";
    ...
    public static final String UPDATE_SHIPPING_PARTNER_BY_ID = "UPDATE shipping_partners SET name = ? WHERE id =?";
    public static final String DISABLE_SHIPPING = "UPDATE shipping_partners SET is_active = false WHERE id =?";
    ...
    public List<ShippingPartner> findAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                partners.add(new ShippingPartner(id, name, is_active));
            }
            conn.commit();
    ...
    public ShippingPartner findById(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                String name = rs.getString("name");
                boolean is_active = rs.getBoolean("is_active");
                shippingPartner = new ShippingPartner(id, name, is_active);
            }
            conn.commit();
        } catch (SQLException e) {
            try {
    ...
    public boolean remove(int id) {
        ...
        try {
            conn = getConnection();
            conn.setAutoCommit(false);
            statement = conn.prepareStatement(DISABLE_SHIPPING);
            statement.setInt(1, id);
